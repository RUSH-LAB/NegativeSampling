...
int Layer<T, Tp>::queryActiveNodeandComputeActivationsOpt(
  ...
  if (Sparsity == 1.0){
    ...
    if (Mode == 1) {
        // Choose up to sparsity * layersize neurons then stop
        std::unordered_set<int> selected;
        int *hashes;
        int *hashIndices;
	int **actives;
        int len = floor(_noOfNodes * Sparsity);

        // Choose a random label to start with
        int la = rand() % labelsize;
            if (HashFunction == 1) {
                hashes = _wtaHasher->getHash(in_values);
            } else if (HashFunction == 2) {
                hashes = _dwtaHasher->getHash( in_indices, _weights+_previousLayerNumOfNodes*label[la],ICI);
            } else if (HashFunction == 4) {
                hashes = _srp->getHashSparse( in_indices, _weights+_previousLayerNumOfNodes*label[la], ICI);
            }
            hashIndices = _hashTables->hashesToIndex(hashes);
            actives = _hashTables->retrieveRaw(hashIndices);

            // Choose a random table to start at
            int rand_table = rand() % labelsize;
            for (int k = 0; k < _L; k++) {
                int i = (rand_table + k) % _L;
                if (actives[i] == NULL) {
                    continue;
                } else {
                    for (int j = 0; j < BUCKETSIZE; j++) {
        ...
        if (_type == NodeType::Softmax) {
            if (labelsize > 0) {
                for (int i=0; i<labelsize ;i++){
                    selected.insert(label[i]);
                }
            }
        }
        in = selected.size();
        if (selected.size()<1500){
            ...
            for (int i = start; i < _noOfNodes; i++) {
                ...
                if (selected.count(_randNode[i]) == 0) {
                    selected.insert(_randNode[i]);
                }
            }
        ...
        int i=0;
        for (auto &&x : selected) {
            _nodeDataOpt[inputID].indices[i] = x;
            i++;
        }
    ...
    if (Mode == 2) {
        ...
        // Sample from tables until sparsity * layersize is reached then stop
        int *hashes;
        int len = floor(_noOfNodes * Sparsity);
